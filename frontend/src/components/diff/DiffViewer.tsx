...
export const DiffViewer: React.FC<DiffViewerProps> = ({ diff, onClose, isInline = false }) => {
  ...
  useEffect(() => {
    ...
    const loadRealDiff = async () => {
      if (!diff.originalContent && !diff.modifiedContent) {
        ...
        try {
          ...
          if (diff.id.startsWith('commit-')) {
            ...
            diffText = await ApiBridge.gitDiff(filePathParam, activeWorkspacePath);
          }
          if (isMounted) {
  ...
  const parsedDiff = useMemo(() => {
    if (diff.originalContent || diff.modifiedContent) {
      ...
      return { isCustom: true, origLines, modLines };
    }

    const text = rawGitDiff;
    if (!text) return { isCustom: false, lines: [] };
    ...
    for (const l of rawLines) {
      if (l.startsWith('diff --git') || l.startsWith('index ') || l.startsWith('---') || l.startsWith('+++')) {
        ...
        lines.push({ type: 'add', text: l.substring(1), modLine: modLineCount++ });
      } else if (l.startsWith('-')) {
        lines.push({ type: 'del', text: l.substring(1), origLine: origLineCount++ });
      } else {
        const clean = l.startsWith(' ') ? l.substring(1) : l;
        lines.push({ type: 'normal', text: clean, origLine: origLineCount++, modLine: modLineCount++ });
      }
    }

    return { isCustom: false, lines };
  }, [rawGitDiff, diff.originalContent, diff.modifiedContent]);
  ...
  return (
    <div className={`flex flex-col bg-white dark:bg-[#1e1e1e] border border-[#e2e8f0] dark:border-[#2b2b2b] rounded-2xl overflow-hidden shadow-xs font-sans ${
      ...
      <div className="px-4 py-2.5 bg-[#f8fafc] dark:bg-[#252526] border-b border-[#e2e8f0] dark:border-[#2b2b2b] flex items-center justify-between flex-wrap gap-2 text-xs select-none">
        <div className="flex items-center gap-2">
          ...
          <span className="flex items-center gap-1 font-mono text-[11px]">
            ...
            <span className="text-[#dc2626] dark:text-[#f87171] font-semibold">-{diff.deletions || 0}</span>
          </span>
        ...
        <div className="flex items-center gap-2">
          ...
          {diff.status === 'pending' && (
            <>
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[#fee2e2] hover:bg-[#fecaca] text-[#dc2626] dark:bg-[#450a0a] dark:hover:bg-[#5c0d0d] dark:text-[#fca5a5] font-semibold text-xs transition-colors cursor-pointer"
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1 rounded-lg bg-[#16a34a] hover:bg-[#15803d] text-white font-semibold text-xs transition-colors shadow-2xs cursor-pointer"
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="p-1 hover:bg-[#e2e8f0] dark:hover:bg-[#333333] rounded text-[#64748b] dark:text-[#a1a1aa] cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          )}
      ...
      <div className="flex-1 overflow-auto font-mono text-[12px] leading-[20px] select-text">
        {isLoadingDiff ? (
          <div className="p-8 text-center text-xs text-[#9ca3af] dark:text-[#666]">
            Loading repository diff...
          </div>
        ) : parsedDiff.isCustom && parsedDiff.origLines && parsedDiff.modLines ? (
          viewMode === 'split' ? (
            <div className="grid grid-cols-2 divide-x divide-[#e2e8f0] dark:divide-[#2b2b2b] min-w-[600px]">
              ...
              <div className="bg-[#fafafa] dark:bg-[#1a1a1a]">
                ...
                <div className="p-2 space-y-0.5">
                  {parsedDiff.origLines.length === 0 ? (
                    <div className="text-[#9ca3af] italic px-2 py-4 select-none">[New File — No Original Content]</div>
                  ) : (
                    parsedDiff.origLines.map((line, idx) => {
                      const isRemoved = !parsedDiff.modLines!.includes(line);
                      return (
                        <div
                          key={idx}
              ...
              <div className="bg-white dark:bg-[#181818]">
                ...
                <div className="p-2 space-y-0.5">
                  {parsedDiff.modLines.map((line, idx) => {
                    const isAdded = !parsedDiff.origLines!.includes(line);
                    return (
                      <div
                        key={idx}
                        className={`flex items-start ${
          ...
          /* Real Unified / Git Hunks Render */
          <div className="p-2 space-y-0.5 bg-white dark:bg-[#181818]">
            {parsedDiff.lines && parsedDiff.lines.length > 0 ? (
              parsedDiff.lines.map((l, idx) => {
                if (l.type === 'header' || l.type === 'file-header') {
                  return (
                    <div key={idx} className="px-3 py-1 bg-[#f1f5f9] dark:bg-[#252528] text-[#2563eb] dark:text-[#60a5fa] font-bold text-[11px] rounded-xs my-1 select-none">
                      {l.text}
                    </div>
                  );
                }
                ...
                return (
                  <div
                    ...
                    className={`flex items-start ${
                      isAdd 
                        ? 'bg-[#dcfce7]/70 dark:bg-[#064e3b]/50 text-[#166534] dark:text-[#86efac]' 
                        : isDel 
                        ? 'bg-[#fee2e2]/60 dark:bg-[#450a0a]/50 text-[#991b1b] dark:text-[#fca5a5]' 
                        : 'text-[#334155] dark:text-[#cccccc]'
                    }`}
                  >
                    <span className="w-10 text-right pr-2 text-[#94a3b8] dark:text-[#555] select-none shrink-0 font-mono text-[11px]">
                      {l.origLine ?? ''}
                    </span>
                    <span className="w-10 text-right pr-2 text-[#94a3b8] dark:text-[#555] select-none shrink-0 font-mono text-[11px]">
                      {l.modLine ?? ''}
                    ...
                    <span className="flex-1 whitespace-pre overflow-x-auto">{l.text || ' '}</span>
                  </div>
                );
              })
            ) : (
              <div className="p-8 text-center text-xs text-[#9ca3af] dark:text-[#666]">
                No difference found. Working tree matches HEAD.
              </div>
            )}
          </div>
        )}
